...
else if (typeof globalthis_default !== "undefined")
  Object.defineProperty(globalthis_default, REFERENCES_KEY, {
    value: INV_REFERENCE,
    configurable: true,
    writable: false,
    enumerable: false,
...
var SERIALIZED_PROMISE_CONSTRUCTOR =
...
var SERIALIZED_STREAM_CONSTRUCTOR =
...
var SERIALIZED_ITERATOR_CONSTRUCTOR =
...
var SERIALIZED_ASYNC_ITERATOR_CONSTRUCTOR =
...
var SERIALIZED_ARRAY_BUFFER_CONSTRUCTOR =
...
async function parsePlainObject$1(ctx, depth, id, current, empty) {
  return createObjectNode(
    ...
    current,
    empty,
    await parseProperties$1(ctx, depth, current),
  );
}
...
function createBaseDeserializerContext(mode, options) {
  ...
  if (!("types" in refs))
    Object.assign(refs, { types: /* @__PURE__ */ new Map() });
  return {
    mode,
    plugins: options.plugins,
    refs,
    features:
      (_options$features = options.features) !== null &&
      _options$features !== void 0
        ? _options$features
    ...
    switch (typeof actual) {
      ...
      case "symbol":
        if (isValidSymbol(actual))
          object[actual] = deserialize$1(ctx, depth, value);
        break;
      default:
...
function deserializeObject(ctx, depth, node) {
  const result = assignIndexedValue$1(
    ctx,
    node.i,
    node.t === 10 ? {} : Object.create(null),
  );
...
function deserializeMap(ctx, depth, node) {
  ...
  for (
    ...
    result.set(
      deserialize$1(ctx, depth, keys[i]),
      deserialize$1(ctx, depth, vals[i]),
    );
  return result;
}
...
function deserializeTypedArray(ctx, depth, node) {
  ...
  const offset =
    (_node$b = node.b) !== null && _node$b !== void 0 ? _node$b : 0;
...
function deserializeDataView(ctx, depth, node) {
  ...
  const source = deserialize$1(ctx, depth, node.f);
  const offset =
    (_node$b2 = node.b) !== null && _node$b2 !== void 0 ? _node$b2 : 0;
  if (offset < 0 || offset > source.byteLength)
    throw new SerovalMalformedNodeError(node);
  return assignIndexedValue$1(
    ctx,
...
function deserializeBoxed(ctx, depth, node) {
  return assignIndexedValue$1(
    ctx,
    node.i,
    Object(deserialize$1(ctx, depth, node.f)),
  );
}
...
function mergeAssignments(assignments) {
  ...
  for (
    let i = 1, len = assignments.length, item, prev = current;
    i < len;
    i++
  ) {
    item = assignments[i];
...
function serializeProperty(ctx, source, key, val) {
  if (typeof key === "string") {
    ...
    const isIdentifier =
      (check >= 0 && check.toString() === key) || isValidIdentifier(key);
    ...
    if (isValidKey(key))
      return (
        (isIdentifier ? key : '"' + key + '"') + ":" + serialize$1(ctx, val)
      );
...
function serializeMapEntry(ctx, id, key, val, sentinel) {
  ...
  if (isIndexedValueInStack(base, key)) {
    ...
    if (val.t !== 4 && val.i != null && isSerializerRefMarked(base, val.i)) {
      const serialized =
        "(" + serialize$1(ctx, val) + ",[" + sentinel + "," + sentinel + "])";
      createSetAssignment(ctx, id, keyRef, getRefParam(ctx, val.i));
  ...
  if (isIndexedValueInStack(base, val)) {
    ...
    if (key.t !== 4 && key.i != null && isSerializerRefMarked(base, key.i)) {
      const serialized =
        "(" + serialize$1(ctx, key) + ",[" + sentinel + "," + sentinel + "])";
...
function serializePromiseConstructor(ctx, node) {
  ...
  return (
    "(" +
    assignIndexedValue(ctx, node.s, getConstructor(ctx, node.f) + "()") +
    ").p"
  );
}
...
function serializeIteratorFactoryInstance(ctx, node) {
  return (
    getConstructor(ctx, node.a[0]) + "(" + serialize$1(ctx, node.a[1]) + ")"
  );
}
...
function serializeTopCross(ctx, tree) {
  ...
  const args =
    scopeId == null ? "()" : '($R["' + serializeString(scopeId) + '"])';
    ...
    valueNodes.push(
      createAsyncIteratorFactoryInstanceNode(
        ...
        parseSOS(
          ...
          ctx.type === 1
            ? createStream()
            : createStreamFromAsyncIterable(properties),
        ),
      ),
    );
  }
  ...
  switch (typeof current) {
    ...
    case "object":
      if (current) {
        const ref = getReferenceNode(ctx.base, current);
        return ref.type === 0
          ? parseObject(ctx, depth + 1, ref.value, current)
          : ref.value;
      }
      return NULL_NODE;
    case "symbol":
...
function fromJSON(source, options = {}) {
  ...
  const sourceFeatures =
    (_source$f = source.f) !== null && _source$f !== void 0 ? _source$f : 127;
